...
def income_finder(ticker, **kwargs):
    ...
    if not params['contractType']:
        raise ValueError(
            " Contract Type of either PUT or CALL should be provided")
...
def filter_strikes(option):
    ...
    def moneyness_flag(option): return ((option.type == PUT_CALL.PUT.value) and
                                        (option.strike_price <= (1 - option.moneyness/100) * option.stock_price) or ((option.type == PUT_CALL.CALL.value) and (option.strike_price >= (1 + option.moneyness/100) * option.stock_price)))

    def premium_flag(
        option): return option.mark > option.desired_premium * option.stock_price/100

    if premium_flag(option) and moneyness_flag(option):
        return True
    else:
        return False